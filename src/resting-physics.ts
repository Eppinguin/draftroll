 ...
 */
export function releaseUnstableRestPose(body: CANNON.Body, correctionAxis: CANNON.Vec3): void {
  if (body.velocity.lengthSquared() >= 0.18 * 0.18) return;
  if (body.angularVelocity.lengthSquared() >= 0.55 * 0.55) return;
  const axisLength = correctionAxis.length();
  if (axisLength < 1e-5) return;
  body.wakeUp();
  correctionAxis.scale(0.22 / axisLength, correctionAxis);
