...
type InternalRoomRollEvent = InternalRollStartEvent | InternalRollUpdatedEvent | InternalRollVisibilityUpdatedEvent;
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!isOriginAllowed(request, env)) {
      return json({ error: 'origin_not_allowed', message: 'The request origin is not allowed by this Draftroll server' }, 403, cors);
    }
    ...
    if (request.method === 'GET' && url.pathname === '/') {
      return json({
        ...
        access: env.ALLOW_ANONYMOUS === 'true' ? 'anonymous-or-capability-token' : 'capability-token',
        endpoints: {
          health: '/health',
    ...
    const roomMatch = /^\/rooms\/([^/]+)\/(connect|state|history|events|diagnostics|policy|rolls\/[^/]+\/revisions)$/.exec(url.pathname);
...
export class DiceRoomObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    participant = {
      ...participant,
      passwordAuthenticated: participant.authorizationComplete
        && (!policy.access.passwordProtected || participant.permissions.includes('room:manage')),
    };
    await this.scheduleMaintenance(policy);
    try {
      assertMetadataSize(participant.metadata, policy.limits.maximumParticipantMetadataBytes, 'participant metadata');
    ...
    if (!isWebSocketUpgrade && !participant.authorizationComplete) {
      return json({ error: 'token_required', message: 'This Draftroll server requires a room capability token' }, 401);
    }
    if (!isWebSocketUpgrade && !participant.passwordAuthenticated) {
      ...
      if (!suppliedPassword) {
        return json({ error: 'room_password_required', message: 'This room requires a password' }, 401);
      }
      try {
        await this.consumeRateLimit(
      ...
      if (!(await this.verifyRoomPassword(suppliedPassword))) {
        return json({ error: 'invalid_room_password', message: 'The room password is incorrect' }, 401);
      }
    ...
    if (url.pathname.endsWith('/state')) {
      ...
      const lastEventSequence = parseNonNegativeInteger(url.searchParams.get('lastEventSequence'), 0);
    ...
    if (url.pathname.endsWith('/events')) {
      if (request.method !== 'GET') return json({ error: 'method_not_allowed' }, 405);
      const afterEventSequence = parseNonNegativeInteger(url.searchParams.get('afterEventSequence'), 0);
    ...
    if (url.pathname.endsWith('/diagnostics')) {
      ...
      if (!participant.permissions.includes('room:manage')) {
        return json({ error: 'permission_denied', message: 'Room diagnostics require room:manage' }, 403);
      }
    ...
    if (revisionsMatch) {
      if (request.method !== 'GET') return json({ error: 'method_not_allowed' }, 405);
      return json(await this.getRevisions(roomId, decodeURIComponent(revisionsMatch[1]), participant));
    }
  ...
  async webSocketMessage(socket: WebSocket, raw: string | ArrayBuffer): Promise<void> {
    ...
    const policy = await this.getPolicy();
    const refreshed: ConnectionAttachment = { ...participant, lastSeenAt: new Date().toISOString() };
    ...
    if (!parsedEvent.event) {
      sendEvent(socket, {
        ...
        message: parsedEvent.error?.message ?? 'Message must be valid JSON with a supported event type',
    ...
    const duplicate = 'requestId' in event
      ? await this.findDuplicateRequest(participant.sessionId, event.requestId)
      : null;
    if (duplicate) {
      const projected = duplicate.type === 'bulk_rolls_updated'
  ...
  async alarm(): Promise<void> {
    ...
    for (const [socket, participant] of this.getSessions()) {
      if (Date.parse(participant.lastSeenAt) < staleBefore) {
        ...
        if (participant.joined) this.broadcastParticipant({
          type: 'participant_left',
          protocolVersion: DRAFTROLL_PROTOCOL_VERSION,
          roomId: participant.roomId,
          participantId: participant.participantId,
    ...
    if (this.listParticipants().length === 0 && idleMs >= policy.lifecycle.roomIdleExpirySeconds * 1_000) {
  ...
  private async getPolicy(): Promise<RoomPolicy> {
    ...
    if (stored) {
      ...
      if (decoded.success) {
        if (JSON.stringify(stored) !== JSON.stringify(decoded.data)) await this.state.storage.put('roomPolicy', decoded.data);
        return decoded.data;
      }
    ...
    policy.access.passwordProtected = Boolean(await this.state.storage.get<StoredRoomPassword>(ROOM_PASSWORD_STORAGE_KEY));
  ...
  private async consumeRateLimit(scope: string, limit: number, limitName: string): Promise<void> {
    const now = Date.now();
    const counters = await this.state.storage.get<Record<string, FixedWindowCounter>>('rateCounters') ?? {};
    const current = counters[scope];
    const counter = !current || now - current.windowStartedAt >= 60_000
  ...
  private assertParticipantCapacity(participant: ConnectionAttachment, policy: RoomPolicy): void {
    const existingSession = this.listParticipants().some((entry) => entry.sessionId === participant.sessionId);
  ...
  private async trackRoll(record: StoredRoll, policy: RoomPolicy): Promise<void> {
    ...
    const removed = updated.splice(0, Math.max(0, updated.length - policy.limits.maximumRollsRetained));
    await this.state.storage.put('rollIndex', updated);
    if (removed.length) await this.state.storage.delete(removed.map((entry) => `roll:${entry.rollId}`));
  }
  ...
  private async recordPersistenceFailure(operation: string, rollId: string | undefined, error: unknown): Promise<void> {
    const failures = await this.state.storage.get<PersistenceFailure[]>('persistenceFailures') ?? [];
  ...
  private async withD1Retry<T>(operation: string, action: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 0; attempt < delays.length; attempt += 1) {
      ...
      try {
        ...
        lastError = error;
        this.logStructured('persistence.retry', { operation, attempt: attempt + 1, message: asError(error).message });
      }
    }
    throw lastError;
  }
  ...
  private async handleParticipantUpdate(
    ...
    assertMetadataSize(event.metadata, policy.limits.maximumParticipantMetadataBytes, 'participant metadata');
    const updated: ConnectionAttachment = {
      ...participant,
  ...
  private async handleNewRoll(
    ...
    if (!policy.authorization.allowParticipantRolls && !participant.permissions.includes('room:manage')) {
      throw new RoomOperationError('policy_denied', 'Room policy does not allow participant-created rolls', 403);
    }
    ...
    const result = event.type === 'display_roll'
    ...
    this.logStructured('roll.created', { roomId: participant.roomId, rollId, sequence, participantId: participant.participantId });
  ...
  private async handleRollUpdate(
    ...
    if (!previous) throw new RoomOperationError('roll_not_found', `Roll '${event.rollId}' was not found in this room`, 404);
    ...
    const internal: InternalRollUpdatedEvent = {
      ...
      serverStartTimeMs: animate ? Date.now() + (startBufferMs ?? MIN_ANIMATION_START_BUFFER_MS) : undefined,
    ...
    await this.recordRequest(participant.sessionId, event.requestId, eventSequence);
    this.broadcastRoll(internal);
    this.logStructured('roll.updated', { roomId: participant.roomId, rollId: event.rollId, revision: result.revision, participantId: participant.participantId, animate, auditLabel: event.audit?.label, auditReason: event.audit?.reason });
    this.state.waitUntil(this.persistRollSafely(record, policy));
  }
  ...
  private async handleBulkRollUpdate(
    ...
    if (event.updates.length === 0) {
      throw new RoomOperationError('invalid_bulk_update', 'Bulk updates require at least one roll', 400);
    }
    ...
    const previousRecords = await Promise.all(event.updates.map((item) => this.loadRoll(item.rollId)));
    const prepared = event.updates.map((item, index) => {
      const previous = previousRecords[index];
      if (!previous) throw new RoomOperationError('roll_not_found', `Roll '${item.rollId}' was not found in this room`, 404);
    ...
    const now = Date.now();
    const internalEvents: InternalRollUpdatedEvent[] = prepared.map(({ item, previous, record }) => {
      ...
      return {
        ...
        animate,
        animationSeed: animate ? crypto.randomUUID() : undefined,
        serverStartTimeMs: animate ? now + (startBufferMs ?? MIN_ANIMATION_START_BUFFER_MS) : undefined,
        animationDurationMs: animate ? 2_800 : undefined,
        startBufferMs,
        audit: item.audit,
      };
    ...
    const environmentLimit = clampInteger(this.env.EVENT_BUFFER_LIMIT ?? null, 10, 10_000, policy.limits.maximumBufferedEvents);
    ...
    requestCache.push({ key: `${participant.sessionId}:${event.requestId}`, eventSequence: acknowledgement.eventSequence });
    ...
    const removed = nextIndex.splice(0, Math.max(0, nextIndex.length - policy.limits.maximumRollsRetained));
    ...
    this.state.waitUntil(this.persistRequestRecord(participant.sessionId, event.requestId, acknowledgement.eventSequence));
    if (removed.length > 0) await this.state.storage.delete(removed.map((entry) => `roll:${entry.rollId}`));
    ...
    if (!projectedAcknowledgement) throw new Error('Bulk acknowledgement requester projection failed');
  ...
  private async handleVisibilityUpdate(
    ...
    await this.recordRequest(participant.sessionId, event.requestId, eventSequence);
    this.broadcastRoll(internal);
    this.logStructured('roll.visibility_updated', { roomId: participant.roomId, rollId: event.rollId, revision: result.revision, participantId: participant.participantId, visibilityType: visibility.type, auditLabel: event.audit?.label, auditReason: event.audit?.reason });
    this.state.waitUntil(this.persistRollSafely(record, policy));
  }
  ...
  private calculateAnimationStartBufferMs(): number {
    ...
    for (const participant of this.getSessions().values()) {
      ...
      const roundTripMs = Number.isFinite(participant.roundTripMs) ? Math.max(0, participant.roundTripMs ?? 0) : 0;
      const uncertaintyMs = Number.isFinite(participant.clockUncertaintyMs) ? Math.max(0, participant.clockUncertaintyMs ?? 0) : 0;
      const readinessPenalty = participant.rendererReady && participant.themesReady ? 0 : 120;
      required = Math.max(
        required,
        Math.ceil(roundTripMs / 2 + uncertaintyMs + CLIENT_RENDER_PREPARATION_BUDGET_MS + readinessPenalty),
      );
    }
    return Math.min(MAX_ANIMATION_START_BUFFER_MS, required);
  }

  private async finalizeSession(socket: WebSocket, participant: ConnectionAttachment): Promise<void> {
  ...
  private async handleTokenAuthentication(
    ...
    try {
      ...
      const updated: ConnectionAttachment = {
        ...
        tokenAudience: payload.aud === undefined
          ? undefined
          : Array.isArray(payload.aud) ? [...payload.aud] : [payload.aud],
        passwordAuthenticated: !policy.access.passwordProtected || permissions.includes('room:manage'),
      };
      assertMetadataSize(updated.metadata, policy.limits.maximumParticipantMetadataBytes, 'participant metadata');
      serializeAttachment(socket, updated);
      this.sessions.set(socket, updated);
  ...
  private async verifyRoomPassword(password: string): Promise<boolean> {
    ...
    const candidate = await deriveRoomPasswordHash(password, base64UrlToBytes(stored.salt), stored.iterations);
  ...
  private async handleRoomPasswordUpdate(
    ...
    if (event.password !== null) {
      ...
      const hash = await deriveRoomPasswordHash(event.password, salt, ROOM_PASSWORD_PBKDF2_ITERATIONS);
    ...
    if (!decodedPolicy.success) throw new RoomOperationError('invalid_policy', decodedPolicy.error.message, 400);
  ...
  private async handleTokenRevocation(
    ...
    if (event.target.type === 'token') {
      if (event.target.expiresAt !== undefined && event.target.expiresAt <= nowSeconds) {
        throw new RoomOperationError('invalid_revocation_target', 'The token expiry must be in the future', 400);
      }
      target = {
        type: 'token',
    ...
    const matchingSessions = [...this.getSessions()].filter(([, session]) =>
      session.tokenAuthenticated && tokenSessionMatchesRevocation(session, target));
    const eventSequence = await this.nextEventSequence();
  ...
  private async handlePolicyUpdate(
    ...
    if (!decodedPatch.success) throw new RoomOperationError('invalid_policy', decodedPatch.error.message, 400);
    const previousPolicy = await this.getPolicy();
    const previousRevision = await this.getPolicyRevision();
    if (event.expectedRevision !== undefined && event.expectedRevision !== previousRevision) {
      throw new PolicyRevisionConflictError(event.expectedRevision, previousRevision);
    ...
    this.broadcastPolicy(internal);
    await this.scheduleMaintenance(decodedPolicy.data);
    this.logStructured('room.policy_updated', { roomId: participant.roomId, revision, participantId: participant.participantId, preset: decodedPolicy.data.preset, enabled: decodedPolicy.data.enabled });
  }
  ...
  private broadcastTokenRevocation(event: InternalRoomTokenRevokedEvent): void {
    for (const [socket, participant] of this.getSessions()) {
      if (!participant.joined || !participant.passwordAuthenticated || !participant.permissions.includes('room:manage')) continue;
      try {
  ...
  private broadcastParticipant(
    event: Extract<ServerToClientEvent, { type: 'participant_joined' | 'participant_updated' | 'participant_left' }>,
    except?: WebSocket,
  ): void {
    for (const [socket, participant] of this.getSessions()) {
  ...
  private async createDiagnostics(roomId: string, policy: RoomPolicy): Promise<Record<string, unknown>> {
    const [eventSequence, eventBuffer, rollIndex, persistenceFailures, policyRevision] = await Promise.all([
    ...
    return {
      ...
      authorizedSessions: sessions.filter((session) => session.authorizationComplete && session.passwordAuthenticated).length,
      ...
      hiddenProjectionEligibleSessions: sessions.filter((session) => session.permissions.includes('roll:view-hidden')).length,
  ...
  private async createRoomState(participant: ConnectionAttachment, lastEventSequence: number): Promise<RoomStateEvent> {
    const eventBuffer = await this.getEventBuffer();
    const eventBufferStartSequence = eventBuffer[0]?.eventSequence ?? (await this.getEventSequence()) + 1;
    const missedEventsTruncated = lastEventSequence > 0 && lastEventSequence < eventBufferStartSequence - 1;
    ...
    const recentRolls = recentEvents.filter((event): event is RoomRollEvent =>
      event.type === 'roll_start' || event.type === 'roll_updated' || event.type === 'roll_visibility_updated');
    const policy = await this.getPolicy();
    return {
      type: 'room_state',
      protocolVersion: DRAFTROLL_PROTOCOL_VERSION,
  ...
  private async getDurableEvents(
    ...
    const query = await this.env.DB.prepare(`
      ...
      LIMIT ?
    `).bind(roomId, afterEventSequence, limit).all<RoomEventRow>();
  ...
  private async getHistory(roomId: string, participant: ConnectionAttachment, limit: number) {
    const query = await this.env.DB.prepare(`
      ...
      LIMIT ?
    `).bind(roomId, limit).all<RollHistoryRow>();

    return {
      roomId,
      rolls: (query.results ?? []).map((row) => projectStoredRoll(
        rowToStoredRoll(row),
        participant,
      )),
    };
  ...
  private async getRevisions(roomId: string, rollId: string, participant: ConnectionAttachment) {
    ...
    `).bind(roomId, rollId).all<RollRevisionRow>();
    return {
      roomId,
      rollId,
  ...
  private async loadRoll(rollId: string): Promise<StoredRoll | null> {
    ...
    const query = await this.env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(rollId, roomId).all<RollHistoryRow>();
  ...
  private async persistEventSafely(event: InternalRoomEvent, policy: RoomPolicy): Promise<void> {
    try {
      ...
      await this.recordPersistenceFailure('persist_event', 'rollId' in event ? event.rollId : undefined, error);
    }
  }

  private async persistEvent(event: InternalRoomEvent, policy: RoomPolicy): Promise<void> {
    const createdAt = new Date().toISOString();
    await this.withD1Retry('room_event_insert', () => this.env.DB.prepare(`
    ...
    `).bind(event.roomId, event.eventSequence, JSON.stringify(event), createdAt).run());
    const cutoff = new Date(Date.now() - policy.lifecycle.historyRetentionSeconds * 1_000).toISOString();
    await this.withD1Retry('room_event_retention', () => this.env.DB.prepare(`
    ...
    `).bind(event.roomId, cutoff, event.roomId, Math.max(policy.limits.maximumBufferedEvents * 20, 1_000)).run());
  }

  private async persistRoll(record: StoredRoll, policy: RoomPolicy): Promise<void> {
    const { result, actor, visibility } = record;
    if (!result.rollId || result.sequence === undefined) throw new Error('Persisted rolls require rollId and sequence');
    const roomId = await this.state.storage.get<string>('roomId') ?? this.state.id.toString();
    await this.withD1Retry('roll_history_upsert', () => this.env.DB.prepare(`
    ...
    await this.withD1Retry('roll_revision_upsert', () => this.env.DB.prepare(`
  ...
  private async enforceD1Caps(roomId: string, rollId: string, policy: RoomPolicy): Promise<void> {
    await this.withD1Retry('revision_cap', () => this.env.DB.prepare(`
    ...
    `).bind(roomId, rollId, roomId, rollId, policy.limits.maximumRevisionsPerRoll).run());

    await this.withD1Retry('room_roll_cap', () => this.env.DB.prepare(`
    ...
    `).bind(roomId, roomId, policy.limits.maximumRollsRetained).run());

    await this.withD1Retry('orphan_revision_cleanup', () => this.env.DB.prepare(`
      DELETE FROM roll_revisions
      WHERE room_id = ? AND roll_id NOT IN (
  ...
  private async assertExactTokenRevocationCapacity(tokenId: string, nowSeconds: number): Promise<void> {
    const exact = await this.state.storage.list<StoredExactTokenRevocation>({
      prefix: TOKEN_REVOCATION_PREFIX,
    });
    const expired = [...exact.entries()]
  ...
  private async cleanupPersistence(policy: RoomPolicy): Promise<void> {
    const roomId = await this.state.storage.get<string>('roomId');
    if (!roomId) return;
    const historyCutoff = new Date(Date.now() - policy.lifecycle.historyRetentionSeconds * 1_000).toISOString();
    const revisionCutoff = new Date(Date.now() - policy.lifecycle.revisionRetentionSeconds * 1_000).toISOString();
    try {
      await this.withD1Retry('history_retention', () => this.env.DB.prepare(`
        DELETE FROM roll_history
        WHERE room_id = ? AND COALESCE(updated_at, created_at) < ?
      `).bind(roomId, historyCutoff).run());
      await this.withD1Retry('revision_retention', () => this.env.DB.prepare(`
        DELETE FROM roll_revisions
        WHERE room_id = ? AND recorded_at < ?
      `).bind(roomId, revisionCutoff).run());
      await this.withD1Retry('event_retention', () => this.env.DB.prepare(`
        DELETE FROM room_events
        WHERE room_id = ? AND created_at < ?
      `).bind(roomId, historyCutoff).run());
      await this.withD1Retry('request_retention', () => this.env.DB.prepare(`
        DELETE FROM room_requests
        WHERE room_id = ? AND created_at < ?
      `).bind(roomId, historyCutoff).run());
      await this.withD1Retry('retention_orphan_cleanup', () => this.env.DB.prepare(`
        DELETE FROM roll_revisions
        WHERE room_id = ? AND roll_id NOT IN (
          SELECT roll_id FROM roll_history WHERE room_id = ?
        )
  ...
  private async recordRequest(sessionId: string, requestId: string, eventSequence: number): Promise<void> {
  ...
  private async persistRequestRecord(sessionId: string, requestId: string, eventSequence: number): Promise<void> {
    const roomId = await this.state.storage.get<string>('roomId') ?? this.state.id.toString();
    const createdAt = new Date().toISOString();
    try {
      await this.withD1Retry('request_idempotency_insert', () => this.env.DB.prepare(`
        ...
        ON CONFLICT(room_id, session_id, request_id) DO UPDATE SET
          ...
          created_at = excluded.created_at
      `).bind(roomId, sessionId, requestId, eventSequence, createdAt).run());
      const cutoff = new Date(Date.now() - 30 * 24 * 60 * 60 * 1_000).toISOString();
      await this.withD1Retry('request_idempotency_retention', () => this.env.DB.prepare(`
        DELETE FROM room_requests
        WHERE room_id = ? AND created_at < ?
  ...
  private async findDuplicateRequest(sessionId: string, requestId: string): Promise<InternalRoomEvent | null> {
    ...
    const requestQuery = await this.env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(roomId, sessionId, requestId).all<RoomRequestRow>();
    ...
    const eventQuery = await this.env.DB.prepare(`
      ...
      WHERE room_id = ? AND event_sequence = ?
      LIMIT 1
    `).bind(roomId, eventSequence).all<RoomEventRow>();
  ...
  private async getRollSequence(): Promise<number> {
    return await this.state.storage.get<number>('rollSequence')
...
function projectEvent(
  ...
  const canViewAudit = isRequester || participant.permissions.includes('room:manage') || participant.permissions.includes('roll:view-hidden');
  const common = {
    protocolVersion: DRAFTROLL_PROTOCOL_VERSION,
    roomId: event.roomId,
    eventSequence: event.eventSequence,
...
function canViewRoll(participant: ConnectionAttachment, actor: RoomActor, visibility: RollVisibility): boolean {
  if (participant.permissions.includes('roll:view-hidden') || participant.permissions.includes('room:manage')) return true;
  ...
  if (visibility.type === 'participants') return visibility.participantIds.includes(participant.participantId);
...
function assertRoomEnabled(participant: ConnectionAttachment, policy: RoomPolicy): void {
  ...
  throw new RoomOperationError(
    'room_disabled',
    policy.shutdownReason ? `Room is disabled: ${policy.shutdownReason}` : 'Room is disabled by policy',
    403,
  );
}
...
function assertVisibilityAllowed(visibility: RollVisibility | undefined, policy: RoomPolicy): void {
  ...
  if (!policy.authorization.allowHiddenRolls) {
    throw new RoomOperationError('hidden_rolls_disabled', 'Room policy does not allow hidden rolls', 403);
  }
  if (normalized.type !== 'roller' && !policy.authorization.allowWhispers) {
    throw new RoomOperationError('whispers_disabled', 'Room policy does not allow participant- or role-targeted rolls', 403);
  }
}
...
function assertRollInputWithinPolicy(
  ...
  if ('expression' in input && typeof input.expression === 'string' && input.expression.length > policy.limits.maximumExpressionLength) {
    throw new RoomOperationError('policy_limit_exceeded', `Expression exceeds room maximum of ${policy.limits.maximumExpressionLength} characters`, 413);
  }
  if ('dice' in input && Array.isArray(input.dice) && input.dice.length > policy.limits.maximumDicePerRoll) {
    throw new RoomOperationError('policy_limit_exceeded', `Roll exceeds room maximum of ${policy.limits.maximumDicePerRoll} dice`, 413);
  }
  if ('operations' in input && Array.isArray(input.operations) && input.operations.length > policy.limits.maximumOperationsPerRoll) {
    throw new RoomOperationError('policy_limit_exceeded', `Roll exceeds room maximum of ${policy.limits.maximumOperationsPerRoll} operations`, 413);
  }
}

function assertRollUpdateWithinPolicy(
  event: { update: Extract<ClientToServerEvent, { type: 'update_roll' }>['update']; reroll?: boolean | string[] },
  policy: RoomPolicy,
): void {
  if (typeof event.update.expression === 'string' && event.update.expression.length > policy.limits.maximumExpressionLength) {
    throw new RoomOperationError('policy_limit_exceeded', `Expression exceeds room maximum of ${policy.limits.maximumExpressionLength} characters`, 413);
  }
  if ((event.update.dice?.length ?? 0) > policy.limits.maximumDicePerRoll
    ...
    throw new RoomOperationError('policy_limit_exceeded', `Update exceeds room maximum of ${policy.limits.maximumDicePerRoll} dice`, 413);
  }
}
...
function assertResultWithinPolicy(result: NormalizedRollResult, policy: RoomPolicy): void {
  if (result.dice.length > policy.limits.maximumDicePerRoll) {
    throw new RoomOperationError('policy_limit_exceeded', `Evaluated result contains ${result.dice.length} dice; room maximum is ${policy.limits.maximumDicePerRoll}`, 413);
  }
  if (result.operations.length > policy.limits.maximumOperationsPerRoll) {
    throw new RoomOperationError('policy_limit_exceeded', `Evaluated result contains ${result.operations.length} operations; room maximum is ${policy.limits.maximumOperationsPerRoll}`, 413);
  }
}

function assertMetadataSize(metadata: Record<string, unknown> | undefined, maximumBytes: number, label: string): void {
  if (!metadata) return;
  const bytes = new TextEncoder().encode(JSON.stringify(metadata)).byteLength;
  if (bytes > maximumBytes) {
    throw new RoomOperationError('policy_limit_exceeded', `${label} is ${bytes} bytes; room maximum is ${maximumBytes}`, 413);
  }
}

function assertPermission(participant: ConnectionAttachment, permission: RoomPermission): void {
  if (participant.permissions.includes(permission) || participant.permissions.includes('room:manage')) return;
  throw new RoomOperationError('permission_denied', `Missing room permission '${permission}'`, 403);
}

function assertRevision(result: NormalizedRollResult, expected: number | undefined, rollId: string): void {
...
async function authorizeRequest(
  ...
  roomId: string,
  env: Env,
  isRevoked: (payload: RoomCapabilityTokenPayload, header: RoomTokenHeader) => boolean | Promise<boolean>,
  ...
  if (token) {
    ...
    throw new AuthorizationError('token_required', 'This Draftroll server requires a room capability token', 401);
  }

  const authorizationComplete = payload !== null || (env.ALLOW_ANONYMOUS === 'true' && !tokenRequested);
  const participantId = payload?.participantId ?? cleanIdentifier(url.searchParams.get('participantId')) ?? crypto.randomUUID();
  const sessionId = payload?.sessionId ?? cleanIdentifier(url.searchParams.get('sessionId')) ?? crypto.randomUUID();
  ...
  return {
    ...
    joined: false,
    passwordAttempts: 0,
    reconnectAfterEventSequence: parseNonNegativeInteger(url.searchParams.get('lastEventSequence'), 0),
...
function parseRoomTokenVerificationKeys(env: Env): RoomTokenVerificationKeySource | null {
  ...
  if (env.ROOM_TOKEN_KEYS?.trim()) {
    ...
    try {
      ...
      throw new AuthorizationError('token_key_configuration_invalid', 'ROOM_TOKEN_KEYS must be a JSON object', 503);
    }
    if (!isRecord(parsed)) {
      throw new AuthorizationError('token_key_configuration_invalid', 'ROOM_TOKEN_KEYS must map key IDs to secrets', 503);
    }
...
function rowToStoredRoll(row: RollHistoryRow): StoredRoll {
  const decoded = decodeNormalizedRollResult(safeJsonParse(row.result_json), { allowLegacyResults: true });
  if (!decoded.success) throw new Error(`Stored roll '${row.roll_id}' is invalid: ${decoded.error.message}`);
  return {
    result: decoded.data,
...
function revisionRowToStoredRoll(row: RollRevisionRow): StoredRoll {
  const decoded = decodeNormalizedRollResult(safeJsonParse(row.result_json), { allowLegacyResults: true });
  if (!decoded.success) throw new Error(`Stored roll revision '${row.revision}' is invalid: ${decoded.error.message}`);
  return {
    result: decoded.data,
    actor: parseActor(row.actor_json),
    visibility: parseVisibility(row.visibility_json),
...
function parseVisibility(value: string | null): RollVisibility {
  ...
  if (isRecord(parsed) && typeof parsed.type === 'string') {
    ...
    if (parsed.type === 'roles' && Array.isArray(parsed.roles)) {
      return { type: 'roles', roles: parsed.roles.filter((role): role is string => typeof role === 'string') };
    }
    if (parsed.type === 'participants' && Array.isArray(parsed.participantIds)) {
      return { type: 'participants', participantIds: parsed.participantIds.filter((id): id is string => typeof id === 'string') };
    }
    if (parsed.type === 'participants-and-roles' && Array.isArray(parsed.participantIds) && Array.isArray(parsed.roles)) {
      return {
        type: 'participants-and-roles',
...
function parseClientEvent(raw: string | ArrayBuffer, maximumBytes: number): { event?: ClientToServerEvent; error?: import('@draftroll/protocol').DraftrollValidationError } {
...
function sendEvent(socket: WebSocket, event: ServerToClientEvent): void {
  const decoded = decodeServerToClientEvent(event, { rejectUnknownFields: true, allowLegacyResults: true });
  if (!decoded.success) {
...
function deserializeAttachment(socket: WebSocket): ConnectionAttachment | null {
  ...
  return {
    ...
    authorizationComplete: value.authorizationComplete === true || value.tokenAuthenticated === true,
    passwordAuthenticated: value.passwordAuthenticated === true,
    joined: value.joined === true,
    tokenAttempts: typeof value.tokenAttempts === 'number' && Number.isSafeInteger(value.tokenAttempts)
      ? Math.max(0, value.tokenAttempts)
      : 0,
    passwordAttempts: typeof value.passwordAttempts === 'number' && Number.isSafeInteger(value.passwordAttempts)
      ? Math.max(0, value.passwordAttempts)
      : 0,
    reconnectAfterEventSequence: typeof value.reconnectAfterEventSequence === 'number' && Number.isSafeInteger(value.reconnectAfterEventSequence)
    ...
    tokenIssuedAt: typeof value.tokenIssuedAt === 'number' && Number.isSafeInteger(value.tokenIssuedAt) ? value.tokenIssuedAt : undefined,
    tokenExpiresAt: typeof value.tokenExpiresAt === 'number' && Number.isSafeInteger(value.tokenExpiresAt) ? value.tokenExpiresAt : undefined,
    ...
    roundTripMs: typeof value.roundTripMs === 'number' && Number.isFinite(value.roundTripMs) ? Math.max(0, value.roundTripMs) : undefined,
    clockUncertaintyMs: typeof value.clockUncertaintyMs === 'number' && Number.isFinite(value.clockUncertaintyMs) ? Math.max(0, value.clockUncertaintyMs) : undefined,
  };
}
...
function toRollError(error: unknown, requestId?: string, rollId?: string): RollErrorEvent {
  if (error instanceof RateLimitError) {
    return {
      type: 'roll_error', requestId, rollId, code: 'rate_limited', message: error.message,
      retryAfterMs: error.retryAfterMs, limit: error.limitName,
    };
  }
  if (error instanceof PolicyRevisionConflictError) {
    return {
      type: 'roll_error', requestId, code: 'policy_revision_conflict', message: error.message,
...
class RateLimitError extends Error {
  constructor(readonly limitName: string, readonly retryAfterMs: number) {
    super(`Rate limit '${limitName}' exceeded; retry in ${Math.ceil(retryAfterMs / 1_000)} seconds`);
...
class PolicyRevisionConflictError extends Error {
  constructor(readonly expectedRevision: number, readonly currentRevision: number) {
    super(`Room policy is at revision ${currentRevision}, not expected revision ${expectedRevision}`);
...
class RevisionConflictError extends Error {
  constructor(
    ...
    super(`Roll '${rollId}' is at revision ${currentRevision}, not expected revision ${expectedRevision}`);
...
function rateLimitedResponse(error: unknown): Response {
  if (error instanceof RateLimitError) {
    ...
    }, 429, { 'retry-after': String(Math.max(1, Math.ceil(error.retryAfterMs / 1_000))) });
  }
  if (error instanceof RoomOperationError) {
...
function isRoomPolicyPreset(value: string | undefined): value is RoomPolicyPreset {
  return value === 'open-table' || value === 'private-gm-table' || value === 'moderated-public-room';
}
