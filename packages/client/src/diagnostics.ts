  ...
  switch (input.code) {
    case 'token_required':
      return [
        {
          code: 'provide-token',
          message: 'Provide a room-scoped capability token.',
    ...
    case 'unknown_key_id':
      return [
        {
          code: 'refresh-token',
          message: 'Obtain a new room capability token from the room host.',
          action: 'replace-token',
        },
      ];
    ...
    case 'invalid_room_password':
      return [
        {
          code: 'provide-password',
          message: 'Provide the current room password.',
      ...
      return [
        {
          ...
          message: 'Upgrade the Draftroll client to a protocol version supported by the room.',
    ...
    case 'message_too_large':
      return [
        {
          code: 'reduce-payload',
          message: 'Reduce dice, metadata, or command size before retrying.',
      ...
      return [
        {
          ...
          message: 'Refresh the roll and retry with the current revision or a merge strategy.',
          action: 'change-request',
        },
      ];
    ...
    case DRAFTROLL_ERROR_CODES.roomConnectionClosed:
      return [
        {
          code: 'check-network',
          message: 'Check network connectivity and the room WebSocket URL.',
          action: 'manual',
        },
    ...
    case 'origin_not_allowed':
      return [
        {
          code: 'check-origin',
          message: 'Ask the host to allow this browser origin.',
          action: 'manual',
        },
      ];
    case undefined:
    default:
      if (input.closeCode === 1006) {
        return [
          {
            code: 'check-network',
            message: 'The socket closed abnormally; check connectivity, proxies, and TLS.',
            action: 'manual',
          },
          ...(input.reconnectEnabled ? [retry()] : []),
        ];
      }
      return input.reconnectEnabled
        ? [retry()]
        : [
            {
              code: 'contact-host',
              message: 'Inspect the room service logs or contact the room host.',
              action: 'manual',
            },
          ];
  }
