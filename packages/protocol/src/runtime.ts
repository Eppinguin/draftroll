 ...
 * @public
 */
export function parseRuntimeJson(raw: string | ArrayBuffer | ArrayBufferView, options: RuntimeJsonOptions = {}): DecodeResult<unknown> {
 ...
 * @public
 */
export function negotiateProtocolVersion(value: unknown): DecodeResult<typeof DRAFTROLL_PROTOCOL_VERSION> {
  if (!Number.isSafeInteger(value)) {
    return failure('Protocol version is invalid', [issue('invalid_protocol_version', '$.protocolVersion', 'Protocol version must be an integer', 'integer', value)]);
  }
 ...
 * @public
 */
export function decodeParticipantIdentityInput(value: unknown, options: DecodeOptions = {}): DecodeResult<ParticipantIdentityInput> {
  const context = createContext({ rejectUnknownFields: true, ...options });
 ...
 * @public
 */
export function decodeRoomCapabilityTokenPayload(value: unknown, options: DecodeOptions = {}): DecodeResult<RoomCapabilityTokenPayload> {
  const context = createContext({ rejectUnknownFields: true, ...options });
  if (expectRecord(value, '$', context)) {
    unknownFields(value, ['protocolVersion', 'roomId', 'participantId', 'sessionId', 'name', 'roles', 'permissions', 'metadata', 'iss', 'aud', 'iat', 'nbf', 'jti', 'exp'], '$', context);
    if (value.protocolVersion !== undefined) validateProtocolVersion(value.protocolVersion, '$.protocolVersion', context);
    ...
    if (value.permissions !== undefined) {
      ...
      if (Array.isArray(value.permissions)) {
        const known = new Set(['roll:create', 'roll:update-own', 'roll:update-any', 'roll:reveal-own', 'roll:reveal-any', 'roll:view-hidden', 'room:manage']);
        value.permissions.forEach((permission, index) => {
          if (typeof permission === 'string' && !known.has(permission)) context.add(issue('invalid_value', `$.permissions.${index}`, 'Unknown room permission', [...known].join(' | '), permission));
        });
      }
    }
    validateOptionalMetadata(value.metadata, '$.metadata', context);
    ...
    if (typeof value.nbf === 'number' && typeof value.exp === 'number' && value.nbf > value.exp) {
      context.add(issue('invalid_value', '$.nbf', 'Token not-before time must not be after expiration', '<= $.exp', value.nbf));
    }
 ...
 * @public
 */
export function decodeCustomDiceDefinitions(value: unknown, options: DecodeOptions = {}): DecodeResult<CustomDiceDefinition[]> {
  const context = createContext({ rejectUnknownFields: true, ...options });
 ...
 * @public
 */
export function decodeNormalizedRollResult(value: unknown, options: DecodeOptions = {}): DecodeResult<NormalizedRollResult> {
  const cloned = isRecord(value) ? structuredClone(value) : value;
  if (isRecord(cloned) && cloned.schemaVersion === undefined && (options.allowLegacyResults ?? true)) {
    cloned.schemaVersion = DRAFTROLL_RESULT_SCHEMA_VERSION;
  }
  const context = createContext({ rejectUnknownFields: true, ...options });
 ...
 * @public
 */
export function assertNormalizedRollResult(value: unknown, options: DecodeOptions = {}): NormalizedRollResult {
 ...
 * @public
 */
export function decodeServerToClientEvent(value: unknown, options: DecodeOptions = {}): DecodeResult<ServerToClientEvent> {
...
/**
 * Determines whether a value is server to client event.
 *
 * @public
 */
export function isServerToClientEvent(value: unknown): value is ServerToClientEvent {
  return decodeServerToClientEvent(value).success;
}

/**
 * Determines whether a value is normalized roll result.
 *
 * @public
 */
export function isNormalizedRollResult(value: unknown): value is NormalizedRollResult {
  return decodeNormalizedRollResult(value).success;
}
...
function validateClientEvent(value: unknown, path: string, context: ValidationContext): void {
  ...
  switch (type) {
    case 'roll_request':
      unknownFields(value, ['type', 'requestId', 'clientRollId', 'input', 'visibility'], path, context);
      ...
      if (value.visibility !== undefined) validateRollVisibility(value.visibility, `${path}.visibility`, context);
      return;
    case 'display_roll':
      unknownFields(value, ['type', 'requestId', 'clientRollId', 'input', 'visibility'], path, context);
      readIdentifier(value.requestId, `${path}.requestId`, context, true);
      ...
      return;
    case 'update_roll':
      unknownFields(value, ['type', 'requestId', 'rollId', 'update', 'expectedRevision', 'reroll', 'animate', 'audit'], path, context);
      readIdentifier(value.requestId, `${path}.requestId`, context, true);
      ...
      if (value.reroll !== undefined && typeof value.reroll !== 'boolean') validateStringArray(value.reroll, `${path}.reroll`, context, context.limits.maximumDice);
    ...
    case 'bulk_update_rolls':
      ...
      readIdentifier(value.requestId, `${path}.requestId`, context, true);
      validateArray(value.updates, `${path}.updates`, context, 50, (item, itemPath, itemContext) => {
        if (!expectRecord(item, itemPath, itemContext)) return;
        unknownFields(item, ['rollId', 'update', 'expectedRevision', 'reroll', 'animate', 'audit'], itemPath, itemContext);
        readIdentifier(item.rollId, `${itemPath}.rollId`, itemContext, true);
        validateRollUpdate(item.update, `${itemPath}.update`, itemContext);
        readOptionalNonNegativeInteger(item.expectedRevision, `${itemPath}.expectedRevision`, itemContext);
        if (item.animate !== undefined) readBoolean(item.animate, `${itemPath}.animate`, itemContext);
        if (item.reroll !== undefined && typeof item.reroll !== 'boolean') validateStringArray(item.reroll, `${itemPath}.reroll`, itemContext, itemContext.limits.maximumDice);
        if (item.audit !== undefined) validateRollAudit(item.audit, `${itemPath}.audit`, itemContext);
      }, true);
      if (Array.isArray(value.updates)) {
        const ids = value.updates.map((item) => isRecord(item) ? item.rollId : undefined).filter((id): id is string => typeof id === 'string');
        if (new Set(ids).size !== ids.length) context.add(issue('invalid_value', `${path}.updates`, 'Bulk updates must contain unique roll IDs', 'unique roll IDs', ids));
      }
      return;
    case 'set_roll_visibility':
      unknownFields(value, ['type', 'requestId', 'rollId', 'visibility', 'expectedRevision', 'audit'], path, context);
      readIdentifier(value.requestId, `${path}.requestId`, context, true);
      readIdentifier(value.rollId, `${path}.rollId`, context, true);
    ...
    case 'revoke_room_token':
      ...
      if (value.reason !== undefined) readString(value.reason, `${path}.reason`, context, { maximum: 500 });
      return;
    case 'clock_sync_ping':
      ...
      return;
    case 'client_ready':
      unknownFields(value, ['type', 'rendererReady', 'themesReady', 'roundTripMs', 'clockUncertaintyMs'], path, context);
...
function validateServerEvent(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  const type = readString(value.type, `${path}.type`, context, { required: true, maximum: 64 });
  if (type !== 'roll_error') validateProtocolVersion(value.protocolVersion, `${path}.protocolVersion`, context);
  switch (type) {
    case 'roll_start':
    case 'roll_updated':
    ...
    case 'bulk_rolls_updated':
      unknownFields(value, ['type', 'protocolVersion', 'roomId', 'requestId', 'eventSequence', 'updates', 'replayed'], path, context);
      ...
      validateArray(value.updates, `${path}.updates`, context, 50, (item, itemPath, itemContext) => {
        ...
        readNonNegativeInteger(item.eventSequence, `${itemPath}.eventSequence`, itemContext, true);
      }, true);
      if (value.replayed !== undefined) readBoolean(value.replayed, `${path}.replayed`, context);
      return;
    case 'clock_sync_pong':
      unknownFields(value, ['type', 'protocolVersion', 'roomId', 'clientTimeMs', 'serverTimeMs', 'nonce'], path, context);
      readIdentifier(value.roomId, `${path}.roomId`, context, true);
      readFiniteNumber(value.clientTimeMs, `${path}.clientTimeMs`, context, true);
    ...
    case 'room_state':
      ...
      return;
    case 'session_ready':
      unknownFields(value, ['type', 'protocolVersion', 'roomId', 'participant', 'latestEventSequence', 'latestRollSequence'], path, context);
      readIdentifier(value.roomId, `${path}.roomId`, context, true);
    ...
    case 'participant_left':
      unknownFields(value, ['type', 'protocolVersion', 'roomId', 'participantId', 'sessionId'], path, context);
      readIdentifier(value.roomId, `${path}.roomId`, context, true);
    ...
    case 'room_token_revoked':
      ...
      return;
    case 'roll_error':
      unknownFields(value, ['type', 'protocolVersion', 'roomId', 'requestId', 'code', 'message', 'rollId', 'currentRevision', 'issues', 'supportedProtocolVersions', 'retryAfterMs', 'limit'], path, context);
      if (value.protocolVersion !== undefined) validateProtocolVersion(value.protocolVersion, `${path}.protocolVersion`, context);
      ...
      if (value.issues !== undefined) validateValidationIssues(value.issues, `${path}.issues`, context);
...
function validateRoomRollEvent(
  ...
  const common = [
    'type', 'protocolVersion', 'roomId', 'eventSequence', 'requestId', 'clientRollId', 'replayed',
    'rollId', 'sequence', 'actor', 'visibility', 'hidden', 'summary', 'result',
  ];
  const extra = type === 'roll_updated'
    ? ['animate', 'animationSeed', 'serverStartTimeMs', 'animationDurationMs', 'startBufferMs', 'audit']
    : type === 'roll_start'
  ...
  if (value.hidden === true && value.result !== null) {
    context.add(issue('invalid_value', `${path}.result`, 'Hidden projections must not contain a result', 'null', value.result));
  }
  if (value.hidden === false && value.result === null) {
    context.add(issue('invalid_value', `${path}.result`, 'Visible roll events require a normalized result', 'NormalizedRollResult', value.result));
  }
  if (type === 'roll_updated') readBoolean(value.animate, `${path}.animate`, context, true);
  if (type === 'roll_visibility_updated') validateProjectedVisibility(value.previousVisibility, `${path}.previousVisibility`, context);
  if (value.audit !== undefined) validateRollAudit(value.audit, `${path}.audit`, context);
  if (value.animationSeed !== undefined) readIdentifier(value.animationSeed, `${path}.animationSeed`, context, true);
  if (value.serverStartTimeMs !== undefined) readFiniteNumber(value.serverStartTimeMs, `${path}.serverStartTimeMs`, context, true);
  if (value.animationDurationMs !== undefined) readRange(value.animationDurationMs, `${path}.animationDurationMs`, context, 0, 120_000);
  if (value.startBufferMs !== undefined) readRange(value.startBufferMs, `${path}.startBufferMs`, context, 0, 10_000);
}

function validateRoomState(value: Record<string, unknown>, path: string, context: ValidationContext): void {
  unknownFields(value, [
    'type', 'protocolVersion', 'roomId', 'sequence', 'latestRollSequence', 'latestEventSequence',
    'eventBufferStartSequence', 'missedEventsTruncated', 'defaultThemeId', 'policy', 'policyRevision',
    'participants', 'recentEvents', 'recentRolls', 'recentRoll',
  ], path, context);
  readIdentifier(value.roomId, `${path}.roomId`, context, true);
  readNonNegativeInteger(value.sequence, `${path}.sequence`, context, true);
  readNonNegativeInteger(value.latestRollSequence, `${path}.latestRollSequence`, context, true);
  readNonNegativeInteger(value.latestEventSequence, `${path}.latestEventSequence`, context, true);
  readNonNegativeInteger(value.eventBufferStartSequence, `${path}.eventBufferStartSequence`, context, true);
  ...
  validateArray(value.recentEvents, `${path}.recentEvents`, context, context.limits.maximumArrayLength, validateReplayEvent, true);
  validateArray(value.recentRolls, `${path}.recentRolls`, context, context.limits.maximumArrayLength, (item, itemPath, itemContext) => {
    if (!expectRecord(item, itemPath, itemContext)) return;
    const eventType = item.type;
    if (eventType !== 'roll_start' && eventType !== 'roll_updated' && eventType !== 'roll_visibility_updated') {
      itemContext.add(issue('invalid_value', `${itemPath}.type`, 'Room state contains a non-roll event', 'room roll event', eventType));
      return;
    }
  ...
  if (value.recentRoll !== undefined) {
    ...
    if (eventType === 'roll_start' || eventType === 'roll_updated' || eventType === 'roll_visibility_updated') {
      ...
      context.add(issue('invalid_value', `${path}.recentRoll.type`, 'recentRoll must be a room roll event', 'room roll event', eventType));
    }
  }
}


function validateRoomPolicyUpdatedEvent(value: Record<string, unknown>, path: string, context: ValidationContext): void {
  unknownFields(value, ['type', 'protocolVersion', 'roomId', 'eventSequence', 'requestId', 'revision', 'actor', 'policy', 'previousPolicy', 'replayed'], path, context);
  readIdentifier(value.roomId, `${path}.roomId`, context, true);
  readNonNegativeInteger(value.eventSequence, `${path}.eventSequence`, context, true);
  readOptionalIdentifier(value.requestId, `${path}.requestId`, context);
...
function validateRoomTokenRevocationTarget(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  const type = readString(value.type, `${path}.type`, context, { required: true, maximum: 64 });
  if (type === 'token') {
  ...
  context.add(issue('invalid_value', `${path}.type`, 'Unknown room token revocation target', 'token | participant', type));
}

function validateRoomTokenRevokedEvent(value: Record<string, unknown>, path: string, context: ValidationContext): void {
  unknownFields(value, ['type', 'protocolVersion', 'roomId', 'eventSequence', 'requestId', 'actor', 'target', 'revokedAt', 'reason', 'disconnectedSessions', 'replayed'], path, context);
  readIdentifier(value.roomId, `${path}.roomId`, context, true);
  readNonNegativeInteger(value.eventSequence, `${path}.eventSequence`, context, true);
  readOptionalIdentifier(value.requestId, `${path}.requestId`, context);
  validateActor(value.actor, `${path}.actor`, context);
...
function validateReplayEvent(value: unknown, path: string, context: ValidationContext): void {
  ...
  if (eventType === 'roll_start' || eventType === 'roll_updated' || eventType === 'roll_visibility_updated') {
    ...
    context.add(issue('invalid_value', `${path}.type`, 'Room state contains an unsupported replay event', 'room roll, policy, or token-revocation event', eventType));
  }
}

function validateRoomPolicy(value: unknown, path: string, context: ValidationContext, patch: boolean): void {
  if (!expectRecord(value, path, context)) return;
  const fullFields = ['schemaVersion', 'preset', 'enabled', 'shutdownReason', 'access', 'authorization', 'limits', 'rateLimits', 'lifecycle', 'renderer'];
  const patchFields = ['preset', 'enabled', 'shutdownReason', 'authorization', 'limits', 'rateLimits', 'lifecycle', 'renderer'];
  unknownFields(value, patch ? patchFields : fullFields, path, context);
  if (!patch) {
    if (value.schemaVersion !== DRAFTROLL_ROOM_POLICY_SCHEMA_VERSION) {
      context.add(issue('invalid_value', `${path}.schemaVersion`, 'Unsupported room-policy schema version', String(DRAFTROLL_ROOM_POLICY_SCHEMA_VERSION), value.schemaVersion));
    }
  }
  ...
  else if (!patch) context.add(issue('missing_field', `${path}.enabled`, 'Room enabled state is required', 'boolean', value.enabled));
  if (value.shutdownReason !== undefined && value.shutdownReason !== null) readString(value.shutdownReason, `${path}.shutdownReason`, context, { maximum: 500 });
...
function validateRollAudit(value: unknown, path: string, context: ValidationContext): void {
  ...
  if (value.reason !== undefined) readString(value.reason, `${path}.reason`, context, { maximum: 500 });
  if (value.label !== undefined) readString(value.label, `${path}.label`, context, { maximum: 100 });
}

function validatePolicyRenderer(value: unknown, path: string, context: ValidationContext, optional: boolean): void {
  ...
  if (value.maximumConcurrentVisuals !== undefined) readBoundedInteger(value.maximumConcurrentVisuals, `${path}.maximumConcurrentVisuals`, context, 1, 250);
  if (value.autoClearMs !== undefined) readBoundedInteger(value.autoClearMs, `${path}.autoClearMs`, context, 0, 24 * 60 * 60 * 1000);
  if (value.reducedMotion !== undefined) readBoolean(value.reducedMotion, `${path}.reducedMotion`, context, true);
  if (value.fallbackOnly !== undefined) readBoolean(value.fallbackOnly, `${path}.fallbackOnly`, context, true);
...
function validatePolicyAccess(value: unknown, path: string, context: ValidationContext, patch: boolean): void {
  if (value === undefined && patch) return;
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['passwordProtected'], path, context);
  if (value.passwordProtected !== undefined) readBoolean(value.passwordProtected, `${path}.passwordProtected`, context, true);
  else if (!patch) context.add(issue('missing_field', `${path}.passwordProtected`, 'Room password protection state is required', 'boolean', value.passwordProtected));
}

function validatePolicyAuthorization(value: unknown, path: string, context: ValidationContext, patch: boolean): void {
  if (value === undefined && patch) return;
  if (!expectRecord(value, path, context)) return;
  const fields = ['allowParticipantRolls', 'allowOwnRollUpdates', 'allowOwnRollRerolls', 'allowOwnRollReveal', 'allowPrivilegedAnyRollUpdates', 'allowPrivilegedAnyRollReveal', 'allowHiddenRolls', 'allowWhispers'];
  unknownFields(value, fields, path, context);
  for (const field of fields) {
    if (value[field] !== undefined) readBoolean(value[field], `${path}.${field}`, context, true);
    else if (!patch) context.add(issue('missing_field', `${path}.${field}`, `Room authorization field '${field}' is required`, 'boolean', value[field]));
  }
}

function validatePolicyLimits(value: unknown, path: string, context: ValidationContext, patch: boolean): void {
  if (value === undefined && patch) return;
  if (!expectRecord(value, path, context)) return;
  ...
  for (const [field, [minimum, maximum]] of Object.entries(ranges)) {
    if (value[field] !== undefined) readBoundedInteger(value[field], `${path}.${field}`, context, minimum, maximum);
    else if (!patch) context.add(issue('missing_field', `${path}.${field}`, `Room limit '${field}' is required`, `${minimum}..${maximum}`, value[field]));
  }
}

function validatePolicyRateLimits(value: unknown, path: string, context: ValidationContext, patch: boolean): void {
  if (value === undefined && patch) return;
  if (!expectRecord(value, path, context)) return;
  const fields = ['connectionAttemptsPerMinutePerIp', 'passwordAttemptsPerMinutePerIp', 'commandsPerMinutePerSession', 'mutationsPerMinutePerParticipant', 'rollsPerMinutePerRoom'];
  unknownFields(value, fields, path, context);
  for (const field of fields) {
    if (value[field] !== undefined) readBoundedInteger(value[field], `${path}.${field}`, context, 1, 100_000);
    else if (!patch) context.add(issue('missing_field', `${path}.${field}`, `Room rate limit '${field}' is required`, '1..100000', value[field]));
  }
}

function validatePolicyLifecycle(value: unknown, path: string, context: ValidationContext, patch: boolean): void {
  if (value === undefined && patch) return;
  if (!expectRecord(value, path, context)) return;
  const ranges: Record<string, [number, number]> = {
  ...
  for (const [field, [minimum, maximum]] of Object.entries(ranges)) {
    if (value[field] !== undefined) readBoundedInteger(value[field], `${path}.${field}`, context, minimum, maximum);
    else if (!patch) context.add(issue('missing_field', `${path}.${field}`, `Room lifecycle field '${field}' is required`, `${minimum}..${maximum}`, value[field]));
  }
}

function validateRoomPassword(value: unknown, path: string, context: ValidationContext, nullable: boolean): void {
  if (nullable && value === null) return;
  const password = readString(value, path, context, { required: true, maximum: 256 });
  if (password !== undefined && password.length < 8) {
    context.add(issue('invalid_value', path, 'Room passwords must contain at least 8 characters', '8..256 characters', password.length));
  }
}
...
function validateRollInput(value: unknown, path: string, context: ValidationContext): void {
  ...
  else context.add(issue('invalid_value', `${path}.mode`, 'Roll input mode is unsupported', 'evaluate | display', value.mode));
}

function validateEvaluateInput(value: Record<string, unknown>, path: string, context: ValidationContext): void {
  unknownFields(value, ['mode', 'name', 'expression', 'dice', 'operations', 'modifier', 'themeId', 'dialect', 'advantage', 'allowComments', 'customDice', 'metadata'], path, context);
  if (value.mode !== 'evaluate') context.add(issue('invalid_value', `${path}.mode`, 'Expected evaluate mode', 'evaluate', value.mode));
  validateCommonRollInput(value, path, context);
  if (value.expression !== undefined) readString(value.expression, `${path}.expression`, context, { maximum: context.limits.maximumExpressionLength });
  ...
  if (value.dice !== undefined) {
    validateArray(value.dice, `${path}.dice`, context, context.limits.maximumDice, validatePlannedDie);
    validateUniqueObjectIds(value.dice, `${path}.dice`, context, 'die');
  }
  if (value.operations !== undefined) validateStructuredOperations(value.operations, `${path}.operations`, context);
  if (value.modifier !== undefined) readFiniteNumber(value.modifier, `${path}.modifier`, context, true);
  if (hasExpression) {
    if (value.operations !== undefined) context.add(issue('invalid_value', `${path}.operations`, 'Expression rolls cannot also define structured operations', 'undefined', value.operations));
    if (value.modifier !== undefined) context.add(issue('invalid_value', `${path}.modifier`, 'Expression rolls cannot also define a structured modifier', 'undefined', value.modifier));
  }
  ...
  if (hasDice) {
    if (value.dialect !== undefined) context.add(issue('invalid_value', `${path}.dialect`, 'Structured dice do not use an expression dialect', 'undefined', value.dialect));
    if (value.allowComments !== undefined) context.add(issue('invalid_value', `${path}.allowComments`, 'Structured dice do not parse expression comments', 'undefined', value.allowComments));
    if (value.advantage !== undefined && value.advantage !== 'none') {
      context.add(issue('invalid_value', `${path}.advantage`, 'Structured dice require explicit keep/drop operations instead of advantage inference', 'none | explicit operation', value.advantage));
    }
  }
}
...
function validateDisplayInput(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['mode', 'name', 'dice', 'total', 'expression', 'themeId', 'annotation', 'comment', 'customDice', 'metadata'], path, context);
  if (value.mode !== 'display') context.add(issue('invalid_value', `${path}.mode`, 'Expected display mode', 'display', value.mode));
  validateCommonRollInput(value, path, context);
  validateArray(value.dice, `${path}.dice`, context, context.limits.maximumDice, validateExternalDie, true);
  validateUniqueObjectIds(value.dice, `${path}.dice`, context, 'die');
...
function validateCommonRollInput(value: Record<string, unknown>, path: string, context: ValidationContext): void {
  if (value.name !== undefined) readString(value.name, `${path}.name`, context, { maximum: 200 });
  readOptionalIdentifier(value.themeId, `${path}.themeId`, context);
  if (value.customDice !== undefined) validateCustomDice(value.customDice, `${path}.customDice`, context);
  validateOptionalMetadata(value.metadata, `${path}.metadata`, context);
}

function validateRollUpdate(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['name', 'expression', 'dice', 'removeDice', 'total', 'modifier', 'annotation', 'comment', 'themeId', 'metadata'], path, context);
  readNullableText(value.name, `${path}.name`, context, 200);
  readNullableText(value.expression, `${path}.expression`, context, context.limits.maximumExpressionLength);
  if (value.dice !== undefined) validateArray(value.dice, `${path}.dice`, context, context.limits.maximumDice, validateDieUpdate);
  if (value.removeDice !== undefined) validateStringArray(value.removeDice, `${path}.removeDice`, context, context.limits.maximumDice);
  if (value.total !== undefined) readFiniteNumber(value.total, `${path}.total`, context, true);
  if (value.modifier !== undefined && value.modifier !== null) readFiniteNumber(value.modifier, `${path}.modifier`, context, true);
  readNullableText(value.annotation, `${path}.annotation`, context);
  readNullableText(value.comment, `${path}.comment`, context);
  if (value.themeId !== undefined && value.themeId !== null) readIdentifier(value.themeId, `${path}.themeId`, context, true);
  validateOptionalMetadata(value.metadata, `${path}.metadata`, context);
}
...
function validateNormalizedResult(value: unknown, path: string, context: ValidationContext): void {
  ...
  unknownFields(value, [
    'schemaVersion', 'rollId', 'sequence', 'revision', 'updatedAt', 'authority', 'name', 'expression',
    'total', 'integerTotal', 'dice', 'operations', 'modifier', 'annotation', 'comment', 'critical', 'dialect',
    'advantage', 'tree', 'themeId', 'metadata', 'customDice', 'createdAt',
  ], path, context);
  if (value.schemaVersion !== DRAFTROLL_RESULT_SCHEMA_VERSION) {
    context.add(issue('invalid_result_schema_version', `${path}.schemaVersion`, 'Unsupported normalized-result schema version', String(DRAFTROLL_RESULT_SCHEMA_VERSION), value.schemaVersion));
  }
  readOptionalIdentifier(value.rollId, `${path}.rollId`, context);
  ...
  if (value.integerTotal !== undefined) readSafeInteger(value.integerTotal, `${path}.integerTotal`, context);
  validateArray(value.dice, `${path}.dice`, context, context.limits.maximumDice, validateNormalizedDie, true);
  validateArray(value.operations, `${path}.operations`, context, context.limits.maximumOperations, validateOperation, true);
  if (value.modifier !== undefined) readFiniteNumber(value.modifier, `${path}.modifier`, context, true);
  readNullableText(value.annotation, `${path}.annotation`, context);
  readNullableText(value.comment, `${path}.comment`, context);
...
function validateNormalizedDie(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['id', 'type', 'sides', 'result', 'numericValue', 'faceIndex', 'faceLabel', 'faceMetadata', 'kept', 'themeId', 'customDiceId', 'appearance', 'physics', 'sourceRollIndex', 'generatedBy', 'generatedFromDieId', 'annotations', 'metadata'], path, context);
  ...
  if (value.numericValue !== undefined) readFiniteNumber(value.numericValue, `${path}.numericValue`, context, true);
  readOptionalNonNegativeInteger(value.faceIndex, `${path}.faceIndex`, context);
  if (value.faceLabel !== undefined) readString(value.faceLabel, `${path}.faceLabel`, context, { maximum: 500 });
  ...
  if (value.annotations !== undefined) validateStringArray(value.annotations, `${path}.annotations`, context, 128);
  validateOptionalMetadata(value.metadata, `${path}.metadata`, context);
}

function validateOperation(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['type', 'count', 'comparator', 'target', 'selector', 'notation', 'appliedTo', 'metadata'], path, context);
  const type = readString(value.type, `${path}.type`, context, { required: true, maximum: 64 });
  ...
  if (value.notation !== undefined) readString(value.notation, `${path}.notation`, context, { maximum: 1_000 });
  validateStringArray(value.appliedTo, `${path}.appliedTo`, context, context.limits.maximumDice, true);
  validateOptionalMetadata(value.metadata, `${path}.metadata`, context);
}

function validateTree(value: unknown, path: string, context: ValidationContext, depth: number): void {
  if (depth > context.limits.maximumTreeDepth) {
    context.add(issue('limit_exceeded', path, 'Result tree exceeds maximum depth', `<= ${context.limits.maximumTreeDepth}`, depth));
    return;
  }
  if (!expectRecord(value, path, context)) return;
  readIdentifier(value.id, `${path}.id`, context, true);
  ...
  if (kind === 'literal') readFiniteNumber(value.literal, `${path}.literal`, context, true);
    ...
    if (!(typeof value.sides === 'number' || typeof value.sides === 'string')) context.add(issue('invalid_type', `${path}.sides`, 'Die tree sides must be numeric or string', 'number | string', value.sides));
    readIdentifier(value.generatedBy, `${path}.generatedBy`, context, true);
  } else if (kind === 'dice') {
    readNonNegativeInteger(value.count, `${path}.count`, context, true);
    if (!(typeof value.sides === 'number' || value.sides === 'F')) context.add(issue('invalid_type', `${path}.sides`, 'Dice tree sides must be numeric or F', 'number | F', value.sides));
    if (value.percentile !== undefined) readBoolean(value.percentile, `${path}.percentile`, context);
    validateArray(value.children, `${path}.children`, context, context.limits.maximumDice, (item, itemPath, itemContext) => validateTree(item, itemPath, itemContext, depth + 1), true);
  } else if (kind === 'set') {
    validateArray(value.children, `${path}.children`, context, context.limits.maximumArrayLength, (item, itemPath, itemContext) => validateTree(item, itemPath, itemContext, depth + 1), true);
  } else if (kind === 'parenthetical' || kind === 'unary') {
    if (kind === 'unary' && value.operator !== '+' && value.operator !== '-') context.add(issue('invalid_value', `${path}.operator`, 'Unary operator is invalid', '+ | -', value.operator));
    ...
    context.add(issue('invalid_value', `${path}.kind`, 'Unknown result tree node kind', 'literal | die | dice | set | parenthetical | unary | binary', kind));
  }
}

function validatePlannedDie(value: unknown, path: string, context: ValidationContext, external = false): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, [
    'id', 'type', 'sides', 'result', 'numericValue', 'faceIndex',
    ...(external ? ['kept', 'sourceRollIndex', 'generatedBy', 'generatedFromDieId'] : []),
    'themeId', 'customDiceId', 'appearance', 'physics', 'annotations', 'metadata',
  ], path, context);
  readIdentifier(value.id, `${path}.id`, context, true);
  readIdentifier(value.type, `${path}.type`, context, true);
  if (value.sides !== undefined) readPositiveInteger(value.sides, `${path}.sides`, context);
  if (value.result !== undefined) validateResultScalar(value.result, `${path}.result`, context, true);
  if (value.numericValue !== undefined) readFiniteNumber(value.numericValue, `${path}.numericValue`, context, true);
  readOptionalNonNegativeInteger(value.faceIndex, `${path}.faceIndex`, context);
  readOptionalIdentifier(value.themeId, `${path}.themeId`, context);
  readOptionalIdentifier(value.customDiceId, `${path}.customDiceId`, context);
...
function validateDieUpdate(value: unknown, path: string, context: ValidationContext): void {
  ...
  for (const key of ['themeId', 'customDiceId'] as const) {
    if (value[key] !== undefined && value[key] !== null) readIdentifier(value[key], `${path}.${key}`, context, true);
  }
  if (value.appearance !== undefined && value.appearance !== null) validateAppearance(value.appearance, `${path}.appearance`, context);
  if (value.physics !== undefined && value.physics !== null) validatePhysicsProperties(value.physics, `${path}.physics`, context);
  if (value.annotations !== undefined && value.annotations !== null) validateStringArray(value.annotations, `${path}.annotations`, context, 128);
  if (value.metadata !== undefined && value.metadata !== null) validateMetadata(value.metadata, `${path}.metadata`, context);
}

function validateStructuredOperations(value: unknown, path: string, context: ValidationContext): void {
  const rankTypes = new Set(['keep-highest', 'keep-lowest', 'drop-highest', 'drop-lowest']);
  const selectorTypes = new Set(['keep', 'drop', 'reroll', 'reroll-once', 'reroll-add', 'explode', 'minimum', 'maximum', 'success-count']);
  validateArray(value, path, context, context.limits.maximumOperations, (item, itemPath, itemContext) => {
    if (!expectRecord(item, itemPath, itemContext)) return;
    unknownFields(item, ['type', 'count', 'dice', 'comparator', 'target', 'selector', 'notation'], itemPath, itemContext);
    const type = readString(item.type, `${itemPath}.type`, itemContext, { required: true, maximum: 64 });
    if (type !== undefined && !rankTypes.has(type) && !selectorTypes.has(type)) {
      itemContext.add(issue('invalid_value', `${itemPath}.type`, 'Unknown structured roll operation', 'known roll operation type', type));
      return;
    }
    if (item.dice !== undefined) {
      validateStringArray(item.dice, `${itemPath}.dice`, itemContext, itemContext.limits.maximumDice);
      validateUniqueStrings(item.dice, `${itemPath}.dice`, itemContext, 'die id');
    }
    if (item.notation !== undefined) readString(item.notation, `${itemPath}.notation`, itemContext, { maximum: 1_000 });

    if (type !== undefined && rankTypes.has(type)) {
      if (item.count !== undefined) readOptionalNonNegativeInteger(item.count, `${itemPath}.count`, itemContext);
      for (const field of ['comparator', 'target', 'selector'] as const) {
        if (item[field] !== undefined) itemContext.add(issue('invalid_value', `${itemPath}.${field}`, `Operation '${type}' uses count rather than ${field}`, 'undefined', item[field]));
      }
      return;
    }

    if (item.count !== undefined) itemContext.add(issue('invalid_value', `${itemPath}.count`, `Operation '${String(type)}' does not accept count`, 'undefined', item.count));
    ...
    if (hasComparator && !['=', '==', '!=', '<', '<=', '>', '>='].includes(String(item.comparator))) {
      itemContext.add(issue('invalid_value', `${itemPath}.comparator`, 'Unknown comparator', '= | == | != | < | <= | > | >=', item.comparator));
    }
    if (hasTarget) readFiniteNumber(item.target, `${itemPath}.target`, itemContext, true);
    if (hasComparator && !hasTarget) itemContext.add(issue('missing_field', `${itemPath}.target`, 'A comparator requires a target', 'finite number', item.target));
    if (!hasSelector && !hasTarget && type !== undefined) {
      itemContext.add(issue('missing_field', itemPath, `Operation '${type}' requires a selector or target`, 'selector | target', item));
    }
    if (hasSelector && (hasComparator || hasTarget)) {
      itemContext.add(issue('invalid_value', itemPath, 'Use either selector or comparator/target shorthand, not both', 'selector | comparator+target', item));
    }
  });
}
...
function validateSelector(value: unknown, path: string, context: ValidationContext): void {
  ...
  const type = readString(value.type, `${path}.type`, context, { required: true, maximum: 64 });
  const supported = ['literal', 'highest', 'lowest', 'greater', 'less', 'greater-equal', 'less-equal', 'not-equal'];
  if (type !== undefined && !supported.includes(type)) {
    context.add(issue('invalid_value', `${path}.type`, 'Unknown roll selector', supported.join(' | '), type));
  }
  if (type === 'highest' || type === 'lowest') readNonNegativeInteger(value.target, `${path}.target`, context, true);
...
function validateCustomDice(value: unknown, path: string, context: ValidationContext): void {
  validateArray(value, path, context, context.limits.maximumCustomDice, (item, itemPath, itemContext) => {
    if (!expectRecord(item, itemPath, itemContext)) return;
    unknownFields(item, ['id', 'faces', 'renderAs', 'metadata'], itemPath, itemContext);
    readIdentifier(item.id, `${itemPath}.id`, itemContext, true);
    validateArray(item.faces, `${itemPath}.faces`, itemContext, itemContext.limits.maximumCustomFaces, (face, facePath, faceContext) => {
      if (!expectRecord(face, facePath, faceContext)) return;
      unknownFields(face, ['result', 'value', 'weight', 'label', 'metadata'], facePath, faceContext);
      validateResultScalar(face.result, `${facePath}.result`, faceContext, true);
      if (face.value !== undefined) readFiniteNumber(face.value, `${facePath}.value`, faceContext, true);
      if (face.weight !== undefined) readPositiveSafeInteger(face.weight, `${facePath}.weight`, faceContext);
      if (face.label !== undefined) readString(face.label, `${facePath}.label`, faceContext, { maximum: 500 });
    ...
    if (Array.isArray(item.faces)) {
      ...
      if (!safe) itemContext.add(issue('invalid_value', `${itemPath}.faces`, 'Custom-die weights must have a positive safe-integer total', `total <= ${Number.MAX_SAFE_INTEGER}`, totalWeight));
    }
...
function validatePhysicsProperties(value: unknown, path: string, context: ValidationContext): void {
  ...
  if (value.sizeScale !== undefined) readRange(value.sizeScale, `${path}.sizeScale`, context, 0.5, 2);
  if (value.massScale !== undefined) readRange(value.massScale, `${path}.massScale`, context, 0.25, 4);
  if (value.inertiaScale !== undefined) readRange(value.inertiaScale, `${path}.inertiaScale`, context, 0.25, 4);
}
...
function validateAppearance(value: unknown, path: string, context: ValidationContext): void {
  ...
  if (value.primaryColor !== undefined) readString(value.primaryColor, `${path}.primaryColor`, context, { maximum: 128 });
  if (value.secondaryColor !== undefined) readString(value.secondaryColor, `${path}.secondaryColor`, context, { maximum: 128 });
...
function validateRollVisibility(value: unknown, path: string, context: ValidationContext): void {
  ...
  if (type === 'public' || type === 'roller') {
    ...
    context.add(issue('invalid_value', `${path}.type`, 'Unknown roll visibility policy', 'public | roller | roles | participants | participants-and-roles', type));
  }
}

function validateProjectedVisibility(value: unknown, path: string, context: ValidationContext): void {
...
function validateActorFields(value: Record<string, unknown>, path: string, context: ValidationContext): void {
  readIdentifier(value.participantId, `${path}.participantId`, context, true);
  readIdentifier(value.sessionId, `${path}.sessionId`, context, true);
...
function validateParticipant(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['participantId', 'sessionId', 'name', 'roles', 'metadata', 'permissions', 'connectedAt'], path, context);
  validateActorFields(value, path, context);
...
function validateRollSummary(value: unknown, path: string, context: ValidationContext): void {
  if (!expectRecord(value, path, context)) return;
  unknownFields(value, ['rollId', 'sequence', 'revision', 'name', 'actor', 'createdAt', 'updatedAt'], path, context);
  readIdentifier(value.rollId, `${path}.rollId`, context, true);
  readNonNegativeInteger(value.sequence, `${path}.sequence`, context, true);
  readNonNegativeInteger(value.revision, `${path}.revision`, context, true);
  if (value.name !== undefined) readString(value.name, `${path}.name`, context, { maximum: 200 });
...
function validateValidationIssues(value: unknown, path: string, context: ValidationContext): void {
  validateArray(value, path, context, 100, (item, itemPath, itemContext) => {
    ...
    readString(item.message, `${itemPath}.message`, itemContext, { required: true, maximum: 2_000 });
    if (item.expected !== undefined) readString(item.expected, `${itemPath}.expected`, itemContext, { maximum: 1_000 });
    if (item.received !== undefined) readString(item.received, `${itemPath}.received`, itemContext, { maximum: 1_000 });
  });
}

function validateProtocolVersion(value: unknown, path: string, context: ValidationContext): void {
  if (!Number.isSafeInteger(value)) {
    context.add(issue('invalid_protocol_version', path, 'Protocol version must be an integer', 'integer', value));
  } else if (value !== DRAFTROLL_PROTOCOL_VERSION) {
    context.add(issue('unsupported_protocol_version', path, `Protocol version ${String(value)} is unsupported`, DRAFTROLL_SUPPORTED_PROTOCOL_VERSIONS.join(' | '), value));
  }
}
...
function validateMetadata(value: unknown, path: string, context: ValidationContext): void {
  ...
  try {
    ...
    context.add(issue('invalid_value', path, 'Metadata must be JSON serializable', 'JSON object', value));
    return;
  }
  const bytes = new TextEncoder().encode(encoded).byteLength;
  if (bytes > context.limits.maximumMetadataBytes) {
    context.add(issue('limit_exceeded', path, 'Metadata exceeds maximum encoded size', `<= ${context.limits.maximumMetadataBytes} bytes`, `${bytes} bytes`));
  }
  const state = { keys: 0 };
  validateJsonValue(value, path, context, 0, state);
}

function validateJsonValue(value: unknown, path: string, context: ValidationContext, depth: number, state: { keys: number }): void {
  if (depth > context.limits.maximumMetadataDepth) {
    context.add(issue('limit_exceeded', path, 'Metadata exceeds maximum nesting depth', `<= ${context.limits.maximumMetadataDepth}`, depth));
    return;
  }
  if (value === null || typeof value === 'boolean' || typeof value === 'string') {
    if (typeof value === 'string' && value.length > context.limits.maximumStringLength) context.add(issue('limit_exceeded', path, 'Metadata string is too long', `<= ${context.limits.maximumStringLength}`, value.length));
    return;
  }
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) context.add(issue('invalid_value', path, 'Metadata numbers must be finite', 'finite number', value));
    return;
  }
  if (Array.isArray(value)) {
    if (value.length > context.limits.maximumArrayLength) context.add(issue('limit_exceeded', path, 'Metadata array is too long', `<= ${context.limits.maximumArrayLength}`, value.length));
    value.forEach((item, index) => validateJsonValue(item, `${path}.${index}`, context, depth + 1, state));
    return;
  }
  if (!isRecord(value)) {
    context.add(issue('invalid_type', path, 'Metadata contains a non-JSON value', 'JSON value', value));
    return;
  }
  for (const [key, item] of Object.entries(value)) {
    state.keys += 1;
    if (state.keys > context.limits.maximumMetadataKeys) {
      context.add(issue('limit_exceeded', path, 'Metadata has too many keys', `<= ${context.limits.maximumMetadataKeys}`, state.keys));
      return;
    }
    if (key.length > 256) context.add(issue('limit_exceeded', `${path}.${key}`, 'Metadata key is too long', '<= 256 characters', key.length));
...
function validateArray(
  ...
  if (!Array.isArray(value)) {
    context.add(issue(required && value === undefined ? 'missing_field' : 'invalid_type', path, 'Expected an array', 'array', value));
    return;
  }
  if (value.length > maximum) context.add(issue('limit_exceeded', path, 'Array exceeds maximum length', `<= ${maximum}`, value.length));
  value.slice(0, maximum + 1).forEach((item, index) => validateItem(item, `${path}.${index}`, context));
}

function validateUniqueObjectIds(value: unknown, path: string, context: ValidationContext, label: string): void {
  if (!Array.isArray(value)) return;
  const seen = new Map<string, number>();
  value.forEach((item, index) => {
    if (!isRecord(item) || typeof item.id !== 'string') return;
    const first = seen.get(item.id);
    if (first !== undefined) context.add(issue('invalid_value', `${path}.${index}.id`, `Duplicate ${label} id '${item.id}'`, `unique id; first used at ${path}.${first}.id`, item.id));
    else seen.set(item.id, index);
  });
}

function validateUniqueStrings(value: unknown, path: string, context: ValidationContext, label: string): void {
  if (!Array.isArray(value)) return;
  const seen = new Map<string, number>();
  value.forEach((item, index) => {
    if (typeof item !== 'string') return;
    const first = seen.get(item);
    if (first !== undefined) context.add(issue('invalid_value', `${path}.${index}`, `Duplicate ${label} '${item}'`, `unique value; first used at ${path}.${first}`, item));
    else seen.set(item, index);
  });
}

function readPositiveSafeInteger(value: unknown, path: string, context: ValidationContext): number | undefined {
  const result = readSafeInteger(value, path, context);
  if (result !== undefined && result < 1) context.add(issue('invalid_value', path, 'Expected a positive safe integer', '1..Number.MAX_SAFE_INTEGER', result));
  return result;
}

function validateStringArray(value: unknown, path: string, context: ValidationContext, maximum: number, required = false): void {
  validateArray(value, path, context, maximum, (item, itemPath, itemContext) => readString(item, itemPath, itemContext, { required: true, maximum: 500 }), required);
}

function validateNumberArray(value: unknown, path: string, context: ValidationContext, maximum: number): void {
  validateArray(value, path, context, maximum, (item, itemPath, itemContext) => readSafeInteger(item, itemPath, itemContext));
}

function validateResultScalar(value: unknown, path: string, context: ValidationContext, required = false): void {
  if (value === undefined && required) context.add(issue('missing_field', path, 'A die result is required', 'number | string', value));
  else if (!(typeof value === 'string' || (typeof value === 'number' && Number.isFinite(value)))) context.add(issue('invalid_type', path, 'Die result must be a finite number or string', 'number | string', value));
  else if (typeof value === 'string' && value.length > context.limits.maximumStringLength) context.add(issue('limit_exceeded', path, 'Die result string is too long', `<= ${context.limits.maximumStringLength}`, value.length));
}

function expectRecord(value: unknown, path: string, context: ValidationContext): value is Record<string, unknown> {
  if (isRecord(value)) return true;
  context.add(issue(value === undefined ? 'missing_field' : 'invalid_type', path, 'Expected an object', 'object', value));
  return false;
}

function unknownFields(value: Record<string, unknown>, allowed: readonly string[], path: string, context: ValidationContext): void {
...
function readString(
  ...
  if (value === undefined) {
    if (options.required) context.add(issue('missing_field', path, 'Required string is missing', 'string', value));
    return undefined;
  }
  ...
  if (value.length > maximum) context.add(issue('limit_exceeded', path, 'String exceeds maximum length', `<= ${maximum}`, value.length));
  return value;
}

function readIdentifier(value: unknown, path: string, context: ValidationContext, required = false): string | undefined {
  const text = readString(value, path, context, { required, maximum: 500 });
  if (text !== undefined && !text.trim()) context.add(issue('invalid_value', path, 'Identifier must not be blank', 'non-empty string', text));
  return text;
}
...
function readBoolean(value: unknown, path: string, context: ValidationContext, required = false): boolean | undefined {
  if (value === undefined) {
    if (required) context.add(issue('missing_field', path, 'Required boolean is missing', 'boolean', value));
    return undefined;
  }
  if (typeof value !== 'boolean') context.add(issue('invalid_type', path, 'Expected a boolean', 'boolean', value));
  return typeof value === 'boolean' ? value : undefined;
}

function readFiniteNumber(value: unknown, path: string, context: ValidationContext, required = false): number | undefined {
  if (value === undefined) {
    if (required) context.add(issue('missing_field', path, 'Required number is missing', 'finite number', value));
    return undefined;
  }
  if (typeof value !== 'number' || !Number.isFinite(value)) context.add(issue('invalid_type', path, 'Expected a finite number', 'finite number', value));
  return typeof value === 'number' && Number.isFinite(value) ? value : undefined;
}

function readSafeInteger(value: unknown, path: string, context: ValidationContext): number | undefined {
  const number = readFiniteNumber(value, path, context, true);
  if (number !== undefined && !Number.isSafeInteger(number)) context.add(issue('invalid_value', path, 'Expected a safe integer', 'safe integer', number));
  return number;
}

function readNonNegativeInteger(value: unknown, path: string, context: ValidationContext, required = false): number | undefined {
  if (value === undefined && !required) return undefined;
  const number = readSafeInteger(value, path, context);
  if (number !== undefined && number < 0) context.add(issue('invalid_value', path, 'Expected a non-negative integer', '>= 0', number));
  return number;
}

function readOptionalNonNegativeInteger(value: unknown, path: string, context: ValidationContext): void {
...
function readPositiveInteger(value: unknown, path: string, context: ValidationContext): void {
  const number = readSafeInteger(value, path, context);
  if (number !== undefined && number < 1) context.add(issue('invalid_value', path, 'Expected a positive integer', '>= 1', number));
}

function readBoundedInteger(value: unknown, path: string, context: ValidationContext, minimum: number, maximum: number): void {
  const number = readSafeInteger(value, path, context);
  if (number !== undefined && (number < minimum || number > maximum)) {
    context.add(issue('invalid_value', path, 'Integer is outside the supported range', `${minimum}..${maximum}`, number));
  }
}

function readRange(value: unknown, path: string, context: ValidationContext, minimum: number, maximum: number): void {
  const number = readFiniteNumber(value, path, context, true);
  if (number !== undefined && (number < minimum || number > maximum)) context.add(issue('invalid_value', path, 'Number is outside the supported range', `${minimum}..${maximum}`, number));
}

function readNullableText(value: unknown, path: string, context: ValidationContext, maximum?: number): void {
  if (value !== undefined && value !== null) readString(value, path, context, { maximum });
}

function readIsoDate(value: unknown, path: string, context: ValidationContext, required = false): void {
  const text = readString(value, path, context, { required, maximum: 64 });
  if (text !== undefined && Number.isNaN(Date.parse(text))) context.add(issue('invalid_value', path, 'Expected an ISO date string', 'ISO-8601 date', text));
}
...
function issue(code: RuntimeValidationCode, path: string, message: string, expected?: string, received?: unknown): RuntimeValidationIssue {
  return {
    code,
    path,
    message,
