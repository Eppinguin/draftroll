...
if (!baseUrlValue) {
  console.error('Set DRAFTROLL_BASE_URL to the deployed Worker origin, for example https://dice.example.com');
  process.exit(2);
}
...
const sessionId = process.env.DRAFTROLL_SMOKE_SESSION_ID ?? `deployment-smoke-${Date.now().toString(36)}`;
...
assert.equal(service.response.status, 200, `service metadata returned HTTP ${service.response.status}`);
...
if (roomId) {
  ...
  assert.equal(state.response.status, 200, `room state returned HTTP ${state.response.status}: ${JSON.stringify(state.body)}`);
...
async function requestJson(url, init) {
  ...
  try {
    ...
    catch { throw new Error(`${url} returned non-JSON content: ${text.slice(0, 200)}`); }
...
function readPositiveInteger(value, fallback) {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0) throw new Error('DRAFTROLL_SMOKE_TIMEOUT_MS must be a positive integer');
  return parsed;
}
