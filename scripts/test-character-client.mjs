...
const [html, client, entry, overlay, overlayHtml, overlayCss, renderer, engine, worker, protocol] =
  await Promise.all([
...
assert.doesNotMatch(
  html,
  /src=["']\/src\/main\.ts["']/,
  'test client must not load the physical engine directly',
);
...
assert.match(
  overlay,
  /visibility:\s*'hidden'/,
  'overlay iframe must be visually inactive while idle',
);
assert.match(
  overlay,
  /this\.setIframeActive\(true\)/,
  'overlay must activate immediately before a roll',
);
assert.match(
  overlay,
  /this\.setIframeActive\(false\)/,
  'overlay must deactivate after dismissal and clear',
);
...
assert.match(
  ...
  'overlay must render with alpha-preserving direct rendering',
);
...
assert.match(
  html,
  /id=["']sdk-room-password["']/,
  'character sheet exposes an optional room-password input',
);
assert.match(
  client,
  /roomPassword:\s*roomPasswordInput\.value/,
  'room password is passed through the SDK options',
);
