...
if (!structureOnly) {
  ...
  try {
    ...
    console.error('TSDoc parser dependencies are unavailable. Run `pnpm install`, or use `pnpm check:docs:structure` only while bootstrapping.');
...
async function walk(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && child.endsWith('.ts') && !child.includes(`${sep}dist${sep}`)) files.push(child);
  }
  return files;
}
...
async function entryPointSources() {
  ...
  for (const packageName of await readdir(packagesRoot)) {
    ...
    for (const target of exportTypeTargets(manifest.exports)) {
      ...
      entries.add(resolve(packageRoot, target.replace('./dist/', './src/').replace(/\.d\.ts$/, '.ts')));
    }
  }
  return entries;
}
...
function validateCommentSyntax(file, source, comment) {
  ...
  for (const message of context.log.messages) {
    ...
    failures.push(`${posix(relative(root, file))}:${location.line}:${location.column} ${message.messageId}: ${message.unformattedText}`);
  }
}
...
function validateDocumentedDeclaration(file, source, sourceFile, node, options = {}) {
  ...
  if (!/@internal\b/.test(comment.text) && summary.length < 8) failures.push(`${label} needs a meaningful summary sentence`);
...
for (const file of sourceFiles) {
  ...
  if (entryPoints.has(file)) {
    if (packageDocs.length !== 1) failures.push(`${relativeFile} must contain exactly one @packageDocumentation comment`);
    if (packageDocs[0]?.start !== 0) failures.push(`${relativeFile} must place @packageDocumentation before imports and exports`);
    ...
    failures.push(`${relativeFile} is not a package entry point and must not use @packageDocumentation`);
  }

  if (relativeFile.endsWith('/instrumentation-internal.ts')) continue;

  for (const statement of sourceFile.statements) {
    if (!isExported(statement) || ts.isExportDeclaration(statement) || ts.isExportAssignment(statement)) continue;
    validateDocumentedDeclaration(file, source, sourceFile, statement, { releaseTag: true });
    coveredDeclarations += 1;
    if (!ts.isClassDeclaration(statement)) continue;
    for (const member of statement.members) {
      if (!isPublicClassMember(member) || ts.isSemicolonClassElement(member) || ts.isPropertyDeclaration(member)) continue;
...
if (failures.length > 0) {
  console.error(`TSDoc validation failed with ${failures.length} issue${failures.length === 1 ? '' : 's'}:`);
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

const parserSummary = structureOnly ? 'parser-free structural mode' : `${parsedComments} parsed comments`;
console.log(`TSDoc validation passed for ${sourceFiles.length} source files, ${coveredDeclarations} exported declarations, and ${coveredMembers} public class members (${parserSummary}).`);
