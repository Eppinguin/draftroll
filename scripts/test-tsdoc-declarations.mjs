...
for (const packageName of await readdir(packagesRoot)) {
  ...
  for (const target of targets) {
    ...
    if (!source.startsWith('/**') || !source.slice(0, 1_000).includes('@packageDocumentation')) {
      failures.push(
        `${manifest.name}: ${target} does not preserve entry-point @packageDocumentation`,
      );
    }
...
for (const [relativeFile, text] of requiredHoverText) {
  ...
  if (!declaration.includes(text))
    failures.push(`${relativeFile} lost required public API documentation`);
}

if (failures.length > 0) {
  console.error(
    `Declaration TSDoc validation failed with ${failures.length} issue${failures.length === 1 ? '' : 's'}:`,
  );
