...
test('documented host and overlay CSP presets allow the cross-origin renderer', async ({
  page,
  browserName,
}) => {
...
test('frame-src policy blocks an unapproved overlay origin and fails closed', async ({ page }) => {
  ...
  await expect
    .poll(() =>
      getState(page).then((state) =>
        state.cspViolations.some((violation) => violation.directive === 'frame-src'),
      ),
    )
    .toBe(true);
  const state = await getState(page);
...
test('connect-src policy blocks room WebSockets without exposing room state', async ({ page }) => {
  ...
  await page.goto(
    `/connect-blocked.html?overlay=0&connect=1&roomId=${roomId}&participantId=csp&sessionId=csp-1&name=CSP`,
  );
  await page.waitForFunction(() => Boolean(window.__draftrollTest));
  await expect(page.getByTestId('status')).toContainText('Error:');
  await expect
    .poll(() =>
      getState(page).then((state) =>
        state.cspViolations.some((violation) => violation.directive === 'connect-src'),
      ),
    )
    .toBe(true);
  const state = await getState(page);
